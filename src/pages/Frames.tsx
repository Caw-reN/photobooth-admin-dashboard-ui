...
console.log("File yang di-drop:", acceptedFiles); 
  ...
  const handleSelectFrame = (frame: any) => {
    ...
    if(frame.coordinates) {
      if(typeof frame.coordinates === 'string') {
        try {
          parsedCoordinates = JSON.parse(frame.coordinates);
        } catch (e) {
          console.error("Format JSON rusak", e);
        }
      } else if (Array.isArray(frame.coordinates)) {
        parsedCoordinates = frame.coordinates;
      }
    }

    setPhotoAreas(parsedCoordinates);

    if(parsedCoordinates.length > 0) {
      setSelectedAreaId(parsedCoordinates[0].id);
    } else {
      setSelectedAreaId('');
    }
  };
  ...
  const handleSaveFrame = async () => {
    ...
    try {
      const isNew = String(selectedFrame.id).startsWith("temp_");
      let responseData;

      if (isNew) {
        responseData = await frameApi.create(formData);
      } else {
        responseData = await frameApi.update(selectedFrame.id, formData);
      }
      
      alert(isNew ? "Mantap! Frame baru berhasil disimpan!" : "Frame berhasil diupdate!");
